...
function getField(record: Record<string, unknown>, ...aliases: string[]): string {
  ...
  for (const [rawKey, value] of Object.entries(record)) {
    ...
    if (
      aliases.some(
        (alias) => normalizeCsvKey(alias).toLowerCase() === normalizedKey.toLowerCase(),
      )
    ) {
      if (typeof value === "string") {
        return value;
...
function buildAssetVersions(records: VersionCsvRecord[]): DemoAssetVersionSeed[] {
  ...
  for (const record of records) {
    ...
    if (!shot) {
      console.warn("[generate-demo-seed] Skipping version row without SHOT", record);
      continue;
    }
    if (!byShot.has(shot)) {
      byShot.set(shot, []);
  ...
  for (const [shot, versions] of byShot) {
    const sorted = versions
      .map((record) => ({
        record,
        versionNumber: Number.parseInt(record["VERSION NUMBER"] ?? "1", 10) || 1,
      }))
    ...
    for (let versionNumber = 1; versionNumber <= maxVersion; versionNumber += 1) {
      ...
      const publishedAt = existing
        ...
        : new Date(basePublishedDate - (maxVersion - versionNumber) * decrementMs)
            .toISOString();
...
function buildStatuses(records: StatusCsvRecord[], prefix: string): DemoStatusSeed[] {
  return records
    .map((record) => {
      const recordMap = record as Record<string, unknown>;
      ...
      const colour = safeTrim(
        record.COLOUR ?? getField(recordMap, "COLOUR", "Colour", "color", "Color"),
      );
      if (!statusName) {
        console.warn("[generate-demo-seed] Skipping status without name", record);
        return null;
      }
      if (!colour) {
        console.warn(
...
function buildPlaylists(
  ...
  for (const record of records) {
    ...
    if (!name) {
      console.warn("[generate-demo-seed] Skipping playlist row without name", record);
      continue;
    }
    if (!typeRaw) {
      console.warn(
    ...
    const categoryName = safeTrim(
      record["PLAYLIST CATEGORY"] ?? getField(recordMap, "PLAYLIST CATEGORY", "Category"),
    );
  ...
  const playlistsByCategory = (category: string, type: "reviewsession" | "list") =>
    playlists.filter((playlist) => {
      if (playlist.type !== type) return false;
      return (playlist.categoryName ?? "").toLowerCase() === category.toLowerCase();
    });
  ...
  const assignToPlaylists = (
    ...
    for (const playlist of playlistIds) {
      playlist.versionIds = versionsByDate
        ...
        .sort((a, b) => a.publishedTimestamp - b.publishedTimestamp || a.shot.localeCompare(b.shot))
  ...
  if (collectionPlaylists.length) {
    ...
    for (const playlist of collectionPlaylists) {
      ...
      playlist.versionIds = versionsByDate
        .filter((version) =>
          new Date(version.publishedAt).toLocaleString("en-US", { month: "long" }).toLowerCase() ===
          monthName,
        )
        .map((version) => version.id);
    }
...
function buildNotes(
  ...
  for (const version of versions) {
    ...
    const createdAt = new Date(baseTimestamp + noteIndex * 60 * 1000).toISOString();
    const labelId = labelCycle.length ? labelCycle[noteIndex % labelCycle.length] : undefined;
...
async function main() {
  ...
  const cliMockRoot = cliRootArg ? cliRootArg.split("=").slice(1).join("=") : undefined;

  const mockRoot =
    cliMockRoot?.trim() ||
    process.env.ASTRANOTES_MOCK_ROOT?.trim() ||
    path.join(process.env.HOME ?? process.cwd(), "Downloads", "AstraNotes_MockData");
  ...
  await Promise.all(Object.values(paths).map((filePath) => ensureFileExists(filePath)));
  ...
  const versionStatuses = buildStatuses(versionStatusRecords, "demo:status:version");
  ...
  console.log(`Generated demo seed with ${assetVersions.length} versions, ${playlists.length} playlists, and ${notes.length} notes.`);
