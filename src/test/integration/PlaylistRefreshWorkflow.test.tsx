...
describe('Playlist Refresh Workflow Integration', () => {
  ...
  describe('Ftrack Playlist Refresh', () => {
    it('should successfully refresh ftrack playlist using ftrackId', async () => {
      ...
      vi.mocked(ftrackService.getPlaylistVersions).mockResolvedValue(mockFtrackVersions);
      ...
      expect(ftrackService.getPlaylistVersions).toHaveBeenCalledWith('ftrack-playlist-123');
      expect(ftrackService.getPlaylistVersions).not.toHaveBeenCalledWith('test-playlist-uuid');
    ...
    it('should detect added and removed versions correctly', async () => {
      ...
      expect(result.current.modifications.addedVersions).toEqual(['version-1', 'version-2']);
      expect(result.current.modifications.removedVersions).toEqual(['old-version']);
    });
  ...
  describe('Local Playlist Refresh', () => {
    it('should skip refresh for local-only playlists', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Cannot refresh local-only playlist: Local Test Playlist');
    });
  ...
  describe('Error Handling', () => {
    it('should handle ftrack API errors gracefully', async () => {
      ...
      vi.mocked(ftrackService.getPlaylistVersions).mockRejectedValue(new Error('API Error'));
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to refresh playlist:', expect.any(Error));
  ...
  describe('Refresh State Management', () => {
    it('should manage isRefreshing state correctly', async () => {
      vi.mocked(ftrackService.getPlaylistVersions).mockImplementation(
        () => new Promise(resolve => setTimeout(() => resolve(mockFtrackVersions), 10))
      );
