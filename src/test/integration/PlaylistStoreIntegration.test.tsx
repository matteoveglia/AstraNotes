...
describe('Playlist Store Integration Tests', () => {
  ...
  describe('Playlist Creation', () => {
    it('should create a local playlist with stable UUID', async () => {
      ...
      expect(createdPlaylist.id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/);
  ...
  describe('Ftrack Metadata Preservation', () => {
    it('should preserve ftrack metadata when creating database entry for ftrack playlist', async () => {
      ...
      await playlistStore.addVersionsToPlaylist(sampleFtrackPlaylist.id, sampleVersions);
  ...
  describe('Version Management', () => {
    ...
    it('should add versions to playlist and store in database', async () => {
      await playlistStore.addVersionsToPlaylist('test-playlist', sampleVersions);

      // Verify versions were stored in database
      const dbVersions = await db.versions.where('playlistId').equals('test-playlist').toArray();
    ...
    it('should remove version from playlist', async () => {
      ...
      // Remove one version
      await playlistStore.removeVersionFromPlaylist('test-playlist', 'version-1');
  ...
  describe('Draft Management', () => {
    beforeEach(async () => {
      ...
      await playlistStore.addVersionsToPlaylist('test-playlist', [sampleVersions[0]]);
    });
    ...
    it('should save and retrieve draft content', async () => {
      ...
      await playlistStore.saveDraft('test-playlist', 'version-1', draftContent, labelId);
      ...
      const retrievedContent = await playlistStore.getDraftContent('test-playlist', 'version-1');
