...
export const TestDataFactory = {
   ...
   */
  createAssetVersions(count: number, overrides: Partial<AssetVersion> = {}): AssetVersion[] {
...
export class TestScenarios {
  ...
  static async setupFtrackPlaylistWithContent(): Promise<{
    ...
    await playlistStore.saveDraft(playlist.id, draftVersionId, 'Test draft content', 'label-123');
  ...
  static async setupMixedContentPlaylist(): Promise<{
    ...
    await playlistStore.addVersionsToPlaylist(playlist.id, [...ftrackVersions, ...manualVersions]);
  ...
  static async setupRefreshScenario(): Promise<{
    ...
    const removedVersions = TestDataFactory.createAssetVersions(1, { id: 'removed-version' });
    const originalVersions = [...unchangedVersions, ...removedVersions];

    // Fresh versions (what comes back from ftrack)
    const addedVersions = TestDataFactory.createAssetVersions(1, { id: 'new-version' });
...
export class TestValidators {
   ...
   */
  static async validateFtrackMetadataInDatabase(playlistId: string, expectedFtrackId: string): Promise<void> {
    ...
    if (dbPlaylist.ftrackId !== expectedFtrackId) {
      throw new Error(`Expected ftrackId ${expectedFtrackId}, got ${dbPlaylist.ftrackId}`);
    }

    if (dbPlaylist.localStatus !== 'synced') {
      throw new Error(`Expected localStatus 'synced', got ${dbPlaylist.localStatus}`);
    }

    if (dbPlaylist.ftrackSyncStatus !== 'synced') {
      throw new Error(`Expected ftrackSyncStatus 'synced', got ${dbPlaylist.ftrackSyncStatus}`);
    }
  }
   ...
   */
  static async validateVersionsInDatabase(playlistId: string, expectedVersionIds: string[]): Promise<void> {
   ...
   */
  static async validateRemovedVersions(playlistId: string, expectedRemovedIds: string[]): Promise<void> {
  ...
  static async validateDraftContent(
    ...
    if (actualContent !== expectedContent) {
      throw new Error(`Expected draft content '${expectedContent}', got '${actualContent}'`);
    }

    if (dbVersion.noteStatus !== expectedStatus) {
      throw new Error(`Expected note status '${expectedStatus}', got '${dbVersion.noteStatus}'`);
    }
  }
}
...
export class TestDatabaseHelpers {
  ...
  static async getDatabaseStats(): Promise<{
    ...
    const publishedCount = versions.filter(v => v.noteStatus === 'published').length;
...
export class TestMockHelpers {
   ...
   */
  static setupFtrackMocks(mock: ReturnType<typeof TestMockHelpers.createFtrackServiceMock>) {
    mock.getPlaylistVersions.mockResolvedValue(TestDataFactory.createAssetVersions(3));
    mock.getPlaylists.mockResolvedValue([TestDataFactory.createFtrackPlaylist()]);
