...
describe("Error Sanitization", () => {
  ...
  describe("sanitizeError", () => {
    ...
    it("should redact multiple credential types", () => {
      const error = new Error("Auth failed: token: bearer123, password: secret123, api-key: key456");
      const sanitized = sanitizeError(error) as Error;
      
      expect(sanitized.message).toBe("Auth failed: token: [REDACTED], password: [REDACTED], api-key: [REDACTED]");
    });
  ...
  describe("safeConsoleError", () => {
    it("should log sanitized errors", () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith("Test message", expect.objectContaining({
