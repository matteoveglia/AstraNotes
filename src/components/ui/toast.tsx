...
interface ToastProviderState {
  toasts: ToastItem[];
  showToast: (message: string, toastType?: ToastType, duration?: ToastDuration) => string;
  ...
  addToBatch: (batchKey: string, message: string, toastType?: ToastType) => void;
  flushBatchedToasts: () => void;
  showApiCredentialsError: (message: string) => string;
  showPlaylistLoadError: (message: string) => string;
  showPublishResult: (successCount: number, failedCount: number, batchKey?: string) => void;
}
...
function useToastProvider(): ToastProviderState {
  ...
  const [batchedToasts, setBatchedToasts] = useState<Record<string, BatchedToast>>({});
  ...
  const showSuccess = useCallback((message: string, duration: ToastDuration = 5000) => {
    return showToast(message, 'success', duration);
  }, [showToast]);
  
  const showError = useCallback((message: string, duration: ToastDuration = 5000) => {
    return showToast(message, 'error', duration);
  }, [showToast]);
  
  const showWarning = useCallback((message: string, duration: ToastDuration = 5000) => {
  ...
  const addToBatch = useCallback((batchKey: string, message: string, toastType: ToastType = 'default') => {
  ...
  const flushBatchedToasts = useCallback(() => {
    Object.entries(batchedToasts).forEach(([key, { toastType, messages }]) => {
      if (messages.length === 1) {
        ...
        showToast(`${messages.length} operations completed: ${messages.length} successful`, toastType);
      }
    });
    setBatchedToasts({});
  ...
  const showApiCredentialsError = useCallback((message: string) => {
    return showPersistentError(`API Connection Error: ${message}`);
  }, [showPersistentError]);
  
  // Playlist loading error
  const showPlaylistLoadError = useCallback((message: string) => {
    return showError(`Failed to load playlist: ${message}`);
  }, [showError]);
  
  // Publish operation notification handling
  const showPublishResult = useCallback((successCount: number, failedCount: number, batchKey: string = 'publish') => {
    if (failedCount === 0) {
      showSuccess(`Successfully published ${successCount} note${successCount !== 1 ? 's' : ''}`);
    } else if (successCount === 0) {
      showError(`Failed to publish ${failedCount} note${failedCount !== 1 ? 's' : ''}`);
    } else {
      showWarning(`Published ${successCount} note${successCount !== 1 ? 's' : ''}, but ${failedCount} failed`);
    }
    ...
    <ToastContext.Provider value={contextValue}>
      <ToastPrimitive.Provider swipeDirection="right">
        ...
        <ToastPrimitive.Viewport
          className="fixed bottom-0 right-0 flex flex-col p-6 gap-2 w-[390px] max-w-[100vw] m-0 list-none z-[9999]"
        />
