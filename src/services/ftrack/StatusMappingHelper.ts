...
export class StatusMappingHelper {
	...
	static async buildSchemaStatusMapping(
		...
		try {
			...
			for (const projectSchema of allProjectSchemas) {
				...
				StatusMappingHelper.schemaStatusMapping[projectSchemaId] = {};
          ...
          where id is "${projectSchemaId}"`,
				...
				if (projectSchemaDetail.asset_version_workflow_schema_id) {
					const assetVersionStatuses =
						await StatusMappingHelper.getStatusesForWorkflowSchema(
							session,
				...
				if (projectSchemaDetail.task_workflow_schema_id) {
					...
					StatusMappingHelper.schemaStatusMapping[projectSchemaId]["Task"] =
						taskStatuses;
				}
			...
			StatusMappingHelper.schemaStatusMappingReady = true;
			console.debug(
				"[StatusMappingHelper] Schema status mapping completed",
				StatusMappingHelper.schemaStatusMapping,
			);
		} catch (error) {
			console.error(
	...
	static async ensureStatusMappingsInitialized(session: any): Promise<void> {
		if (StatusMappingHelper.schemaStatusMappingReady) {
			return;
		}

		console.debug("[StatusMappingHelper] Initializing status mappings");
		const sharedData = await StatusMappingHelper.fetchSharedStatusData(session);
		await StatusMappingHelper.buildSchemaStatusMapping(session, sharedData);
	}
	...
	static async getStatusesForObjectType(
		...
		await StatusMappingHelper.ensureStatusMappingsInitialized(session);
		...
		for (const projectSchemaId in StatusMappingHelper.schemaStatusMapping) {
	...
	static clearCache(): void {
		StatusMappingHelper.schemaStatusMapping = {};
		StatusMappingHelper.schemaStatusMappingReady = false;
		StatusMappingHelper.allWorkflowSchemas = [];
