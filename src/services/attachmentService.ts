...
import { type Session, SERVER_LOCATION_ID } from "@ftrack/api";
...
export class AttachmentService {
	...
	static async uploadAttachment(
		...
		try {
			...
			const baseUrl = AttachmentService.getServerUrl(session);
			const authHeaders = AttachmentService.getAuthHeaders(session);
	...
	static validateAttachments(attachments: Attachment[]): {
		...
		attachments.forEach((attachment) => {
			const validation = AttachmentService.validateAttachment(attachment);
	...
	static async uploadWebUIStyleAttachment(
		...
		try {
			...
			const serverUrl = AttachmentService.getServerUrl(session);
			const apiHeaders = AttachmentService.getAuthHeaders(session);
		...
		try {
			...
			const results = await Promise.all(
				attachments.map(async (attachment) => {
					const result = await AttachmentService.uploadWebUIStyleAttachment(
						session,
						attachment,
						(progress) => {
							if (onProgress) {
			...
			if (componentIds.length > 0) {
				...
				const linkResult = await AttachmentService.attachComponentsToNote(
					session,
					noteId,
					componentIds,
				);
	...
	static async createNoteWithAttachmentsAPI(
		...
		try {
			...
			const validation = AttachmentService.validateAttachments(attachments);
			if (!validation.valid) {
				console.warn(
			...
			const results = await Promise.all(
				attachments.map(async (attachment, index) => {
					try {
						const result =
							await AttachmentService.uploadAttachmentWithCreateComponent(
								session,
								attachment,
								(progress) => {
									if (onProgress) {
