...
export function useNotePublishing(
  ...
  const publishSelected = useCallback(async (selectedVersions: string[]) => {
    if (selectedVersions.length === 0) return { success: false, message: 'No versions selected' };
  ...
  // Publish all notes
  const publishAll = useCallback(async (currentVersionIds?: Set<string>) => {
    try {
      setIsPublishing(true);
      ...
      const publishPromises = Object.entries(noteDrafts)
        ...
        .filter(([versionId]) => !currentVersionIds || currentVersionIds.has(versionId)) // Only publish notes for versions in current playlist
