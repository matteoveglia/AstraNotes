...
describe('network utilities', () => {
  ...
  describe('apiWithNotifications', () => {
    it('should show success toast when all items succeed', async () => {
      ...
      await result.current.apiWithNotifications(
        mockApiFn,
        [{ id: 1 }, { id: 2 }]
      );
    ...
    it('should handle errors and return empty success array', async () => {
      ...
      // Call apiWithNotifications
      const response = await result.current.apiWithNotifications(
        mockApiFn,
        [{ id: 1 }, { id: 2 }]
      );
      
      // Verify error handling
      expect(consoleSpy).toHaveBeenCalledWith('API operation failed:', mockError);
