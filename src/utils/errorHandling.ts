...
export function useErrorHandler() {
  ...
  const handleError = (error: unknown, context: string = '') => {
    ...
    const { isNetworkError, isAuthError, message, name } = categorizeError(error);
    
    if (isNetworkError) {
      toast.showError(`Connection Error: Unable to connect to ftrack API. Please check your internet connection.`);
    } else if (isAuthError) {
      if (name === 'ServerError' || message.includes('Invalid username') || message.includes('ftrack-user')) {
        toast.showError(`Authentication Error: Invalid username or API key. Please check your ftrack credentials in settings.`);
      } else {
