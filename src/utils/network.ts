...
export function useApiWithNotifications() {
   ...
   */
  const apiWithNotifications = useCallback(async <T, R>(
    ...
    try {
      ...
      toast.showError(`Operation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
   ...
   */
  const publishWithNotifications = useCallback(<T extends { versionId: string }>(
    apiFn: (items: T[]) => Promise<{ success: T[], failed: T[] }>,
    items: T[]
  ) => {
    return apiWithNotifications(apiFn, items, {
      successMessage: (result) => `Successfully published ${result.success.length} note${result.success.length !== 1 ? 's' : ''}`,
      errorMessage: (result) => `Failed to publish ${result.failed.length} note${result.failed.length !== 1 ? 's' : ''}`,
      warningMessage: (result) => `Published ${result.success.length} note${result.success.length !== 1 ? 's' : ''}, but ${result.failed.length} failed`
    });
