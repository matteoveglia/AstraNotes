...
export class PlaylistRepository implements PlaylistOperations {
  ...
  async addVersionToPlaylist(playlistId: string, version: VersionEntity): Promise<void> {
    const record: VersionRecord = this.versionEntityToRecord(version);
    await db.versions.add(record);
    
    console.log(`[PlaylistRepository] Added version ${version.id} to playlist ${playlistId}`);
  }
  ...
  async updateVersion(
    ...
    const updated = await db.versions.update([playlistId, versionId], updateRecord);
    if (updated === 0) {
      throw new Error(`Version ${versionId} not found in playlist ${playlistId}`);
    }
    
    console.log(`[PlaylistRepository] Updated version ${versionId} in playlist ${playlistId}`, updates);
  }
  ...
  async bulkUpdateVersions(
    ...
    console.log(`[PlaylistRepository] Bulk updated versions for playlist ${playlistId}`, updates);
  }
