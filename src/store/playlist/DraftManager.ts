...
export class DraftManager implements DraftOperations {
  ...
  async saveDraft(
    ...
    console.log(`[DraftManager] Saving draft for version ${versionId} in playlist ${playlistId}`, {
  ...
  async clearDraft(playlistId: string, versionId: string): Promise<void> {
    console.log(`[DraftManager] Clearing draft for version ${versionId} in playlist ${playlistId}`);
  ...
  async publishNote(playlistId: string, versionId: string): Promise<void> {
    console.log(`[DraftManager] Publishing note for version ${versionId} in playlist ${playlistId}`);
   ...
   */
  async getPlaylistDrafts(playlistId: string): Promise<Array<{
    versionId: string;
    content: string;
    labelId?: string;
    lastModified: number;
    ...
    console.log(`[DraftManager] Found ${drafts.length} drafts in playlist ${playlistId}`);
    return drafts;
  }
  ...
  async clearAllDrafts(playlistId: string): Promise<number> {
    ...
    const versions = await this.repository.getPlaylistVersions(playlistId);
    const draftsToClr = versions.filter(v => v.draftContent && v.draftContent.trim());
    ...
    console.log(`[DraftManager] Cleared ${draftsToClr.length} drafts in playlist ${playlistId}`);
  ...
  async publishAllDrafts(playlistId: string): Promise<number> {
    console.log(`[DraftManager] Publishing all drafts in playlist ${playlistId}`);
    
    const versions = await this.repository.getPlaylistVersions(playlistId);
    const draftsToPublish = versions.filter(v => 
    ...
    console.log(`[DraftManager] Published ${draftsToPublish.length} drafts in playlist ${playlistId}`);
  ...
  async getDraftStats(playlistId: string): Promise<{
    ...
    const stats = {
      ...
      publishedNotes: versions.filter(v => v.noteStatus === 'published').length,
      reviewedNotes: versions.filter(v => v.noteStatus === 'reviewed').length,
    };
    
    console.log(`[DraftManager] Draft stats for playlist ${playlistId}:`, stats);
    return stats;
  }
   ...
   */
  async searchDrafts(playlistId: string, searchTerm: string): Promise<Array<{
    versionId: string;
    versionName: string;
    content: string;
    matchStart: number;
    ...
    console.log(`[DraftManager] Found ${results.length} draft matches for "${searchTerm}" in playlist ${playlistId}`);
    return results;
  }
} 