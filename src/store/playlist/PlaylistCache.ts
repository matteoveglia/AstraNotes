...
import type { VersionEntity, CacheOperations, CacheConfig } from "./types";
...
export class PlaylistCache implements CacheOperations {
	...
	getVersions(playlistId: string): VersionEntity[] | null {
		...
		entry.lastAccessed = Date.now();
		console.log(
			`[PlaylistCache] Cache hit for versions: ${playlistId} (${entry.data.length} versions)`,
		);
		return entry.data;
	}
	...
	setVersions(playlistId: string, versions: VersionEntity[]): void {
		...
		console.log(
			`[PlaylistCache] Cached ${versions.length} versions for playlist: ${playlistId}`,
		);
	}
	...
	clear(): void {
		...
		console.log(
			`[PlaylistCache] Cleared cache (${playlistCount} playlists, ${versionCount} version entries)`,
		);
	}
	...
	getStats() {
		return {
			playlists: {
				...
				entries: Array.from(this.playlistCache.entries()).map(
					([id, entry]) => ({
						id,
	...
	private ensureCacheSize<T>(cache: Map<string, CacheEntry<T>>): void {
		if (cache.size >= this.config.maxEntries) {
			...
			console.log(
				`[PlaylistCache] Removing ${toRemove} LRU entries from cache`,
			);
	...
	private cleanupExpired(): void {
		...
		if (removedCount > 0) {
			console.log(
				`[PlaylistCache] Cleaned up ${removedCount} expired cache entries`,
			);
		}
	}
